...
config_data = {
    'incident'  : {
        ...
        'fields'  :  [{
                ...
                'en_itop' : [''],
                'es_itop' : ['Ref','Descripción','Solución', 'Bitácora Pública', 'Bitácora Privada'],
                'ivanti'  : 'Symptom',
                'require_catalog' : False
            },{
                'en_itop' : [''],
    ...
    'change' : {
        ...
        'fields'  :  [{
                ...
                'en_itop' : [''],
                'es_itop' : ['Ref','Descripción','Motivo','Motivo de Cancelado','Plan en caso de Falla', 'Bitácora Privada'],
                'ivanti'  : 'Description',
                'require_catalog' : False
            },{
                'en_itop' : [''],
...
def combine_columns(row): 
    return f"""Ticket: {safe_get(row.get('ticket', ''))}

Descripción: 
{safe_get(row.get('description', ''))}

Solución: 
{safe_get(row.get('solution', ''))}

Bitácora publica: 
{safe_get(format_log(row.get('public_log', '')))}

Bitácora privada: 
{safe_get(format_log(row.get('private_log', '')))}
"""

def combine_columns_change(row):
        return f"""Ticket: {safe_get(row.get('ticket', ''))}
...
Motivo: 
{safe_get(row.get('reason', ''))}

Motivo de Cancelado: 
{safe_get(row.get('cancel_reason', ''))}

Plan en caso de Falla: 
{safe_get(row.get('failure_plan', ''))}
...
def save_document(document: pd.DataFrame , _filename: str):
    ...
    document.to_csv("{}/{}.csv".format(output_route,_filename),index=False, encoding='UTF-8')
...
def create_incident_import_file(incident_df: pd.DataFrame):
    ...
    for field in fields:
        if field['es_itop'][0] in incident_df.columns:
            if field['ivanti'] == "Symptom":
                export_document_df[field['ivanti']] = incident_df.apply(
                    lambda row: combine_columns({
                        'ticket': row[field['es_itop'][0]],
                        'description': row[field['es_itop'][1]],
                        'solution': row[field['es_itop'][2]],
                        'public_log': row[field['es_itop'][3]],
                        'private_log': row[field['es_itop'][4]]
                    }), axis=1)
            else:
...
def create_request_import_file(service_request_df: pd.DataFrame):
    ...
    for field in fields:
        if field['es_itop'][0] in service_request_df.columns:
            if field['ivanti'] == "Symptom":
                export_document_df[field['ivanti']] = service_request_df.apply(
                    lambda row: combine_columns({
                        'ticket': row[field['es_itop'][0]],
                        'description': row[field['es_itop'][1]],
                        'solution': row[field['es_itop'][2]],
...
def create_change_request_import_file(change_request_df: pd.DataFrame):
    ...
    for field in fields:
        if field['es_itop'][0] in change_request_df.columns:
            if field['ivanti'] == "Description":
                export_document_df[field['ivanti']] = change_request_df.apply(
                    lambda row: combine_columns_change({
                        'ticket': row[field['es_itop'][0]],
                        'description': row[field['es_itop'][1]],
                        'reason': row[field['es_itop'][2]],
                        'cancel_reason': row[field['es_itop'][3]],
                        'failure_plan': row[field['es_itop'][4]],
                        'private_log': row[field['es_itop'][5]]
                    }), axis=1)
            else:
...
def process_input(args):
    ...
    if (args.servicerequest): 
        if not args.servicerequest.lower().endswith('.xlsx'):
            print("Error: El archivo de service request debe ser un archivo XLSX.")
            sys.exit(1)
        try:
            #service_request_df = pd.read_excel(args.servicerequest, index_col=False ,sheet_name='Sheet1', encoding='iso-8859-1')
            service_request_df = pd.read_excel(open(args.servicerequest, 'rb'), index_col=False, sheet_name='Sheet1')
            create_request_import_file(service_request_df)
        except Exception as e:
            print(f"Error al leer el archivo XLSX: {e}")
            sys.exit(1)
    if (args.incident):
        if not args.incident.lower().endswith('.xlsx'):
            print("Error: El archivo de incident debe ser un archivo XLSX.")
            sys.exit(1)
        try:
            #incident_df = pd.read_excel(args.incident, index_col=False, sheet_name='Sheet1', encoding='iso-8859-1')
            incident_df = pd.read_excel(open(args.incident, 'rb'), index_col=False, sheet_name='Sheet1')
            create_incident_import_file(incident_df)
        except Exception as e:
            print(f"Error al leer el archivo XLSX: {e}")
            sys.exit(1)
    if (args.change):
        if not args.change.lower().endswith('.xlsx'):
            print("Error: El archivo de change request debe ser un archivo XLSX.")
            sys.exit(1)
        try:
            #change_request_df = pd.read_excel(args.change, index_col=False, sheet_name='Sheet1')
            change_request_df = pd.read_excel(open(args.change, 'rb'), index_col=False, sheet_name='Sheet1')
